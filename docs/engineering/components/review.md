...
- **Embedded review instructions** (Ralph-owned): instruct the AI to evaluate the user’s prompt along the four dimensions (O005/R007) — signal and state (including whether the prompt instructs the AI to emit the outcome signal on the last line), iteration awareness (Ralph’s preamble explains the loop; review checks that the prompt avoids pass-count logic and that state is re-read compatible), scope and convergence, subjective completion — and to create the five report files on disk.
