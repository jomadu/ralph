...
Scripts and CI need to gate on review outcome by reading result.json in the report directory (or using process exit code). The system uses distinct exit codes (or a documented scheme) so that:

- **The documented success code:** Review completed; report directory (including result.json, revision.md, etc.) and suggested revision were produced; the prompt has no reported errors (or only minor/acceptable issues per product definition). Apply, if requested, completed if applicable.
- **The documented prompt-errors code:** Review completed; report directory (including result.json, revision.md, etc.) and suggested revision were produced; the prompt has one or more errors or serious issues according to the evaluation dimensions. Apply, if requested, may still have been performed; the code indicates "review succeeded but prompt has problems."
...
| Report directory unwritable or path is existing file (R005) | The documented failure code is used. |
...
**Expected output:** The documented success code is used. Report directory exists and contains result.json and other report files; revision is available. CI can treat as pass.
...
**Expected output:** The documented prompt-errors code is used. Report directory exists and contains result.json and summary.md describing the issues; revision suggests fixes. CI can treat as "prompt needs work."
...
- [ ] When the review completes and produces report directory and revision, the exit code is either the documented success code or the documented prompt-errors code depending on whether the prompt has reported errors; the documented failure code is not used in that case.
