...
- **Optional wrapper:** A wrapper script (e.g. `scripts/cursor-wrapper.sh` in the Ralph repo) that invokes the same agent command, parses JSON with `jq`, and emits plain text to stdout may be used as a workaround. Users who want plain-text stdout can set `ai_cmd` or a user-defined alias to the path of that script. The wrapper is not an embedded resource; behavior is as specified here.
