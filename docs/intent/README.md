...
| [O5](./O5-prompt-review/README.md) | Ralph prompts can be reviewed for quality and structure before or without running the loop, with actionable feedback, an optional suggested revision of the prompt, and a machine-readable result; the user may apply the revision to the prompt file with confirmation or non-interactive `--apply -y` | User runs `ralph review <alias>` or `ralph review -f <path>`; receives a report, optionally including a full suggested revision; with `--apply`, is prompted to apply it (or uses `-y` to apply without prompt); exit code 0 (no errors), 1 (errors), or 2 (review failed to run or apply invalid) |
