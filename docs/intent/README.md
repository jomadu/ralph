...
| [O5](./O5-prompt-review/README.md) | Prompts can be reviewed for quality and structure before or without running the loop; report and optional revision; `--apply` writes revision to the prompt file | User runs `ralph review <alias>`, `ralph review -f <path>`, or pipes prompt via stdin; gets report (optional revision); `--apply` applies to file (confirm or `-y`); exit 0 (ok), 1 (errors in prompt), 2 (review/apply failed) |
| [O6](./O6-install-uninstall/README.md) | Users can install Ralph on their system and uninstall it cleanly | User follows documented steps to install (e.g. binary on PATH or known location); invokes `ralph` from a shell; follows documented steps to uninstall with no broken references or leftover artifacts |
| [O7](./O7-user-facing-documentation/README.md) | Users have access to documentation that enables them to use Ralph effectively | User can find and use user-facing docs (how-to, workarounds, backends); docs are maintained and aligned with product behavior so users succeed without reverse-engineering |
