...
A crash does not receive special treatment beyond what the loop already provides. The partial output is scanned for signals using the same rules as a normal exit. If no signal is found, the iteration counts as a no-signal iteration — the consecutive failure counter is not incremented, and the loop proceeds to the next iteration. A crash counts as one completed iteration toward the max iteration limit.
...
- [ ] A crash with no signal in the partial output does not increment the consecutive failure counter
